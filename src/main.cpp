...
CaptureDeviceInfo select_capture_device() {
    ...
    for (const auto& device : devices) {
        if (device.symbolic_link.find(kCaptureHardwareId) != std::wstring::npos) {
            std::wcout << L"Selected capture device: " << device.name << L" (VID_345F:PID_2131)\n\n";
            return device;
        }
    }
...
void create_render_target() {
    ...
    throw_if_failed(g_d3d.swap_chain->GetBuffer(0, IID_ID3D11Texture2D, reinterpret_cast<void**>(&back_buffer)),
                    "Failed to get swap-chain back buffer");
    const HRESULT view_result = g_d3d.device->CreateRenderTargetView(back_buffer, nullptr, &g_d3d.render_target);
    back_buffer->Release();
...
void resize_swap_chain(UINT width, UINT height) {
    ...
    throw_if_failed(g_d3d.swap_chain->ResizeBuffers(0, width, height, DXGI_FORMAT_UNKNOWN, 0),
...
void initialize_video_renderer() {
    ...
    const HRESULT vertex_result = g_d3d.device->CreateVertexShader(vertex_blob->GetBufferPointer(), vertex_blob->GetBufferSize(), nullptr, &g_d3d.video_vertex_shader);
    vertex_blob->Release();
    ...
    const HRESULT pixel_result = g_d3d.device->CreatePixelShader(pixel_blob->GetBufferPointer(), pixel_blob->GetBufferSize(), nullptr, &g_d3d.video_pixel_shader);
    pixel_blob->Release();
    ...
    throw_if_failed(g_d3d.device->CreateTexture2D(&texture_desc, nullptr, &g_d3d.yuy2_texture), "Failed to create YUY2 upload texture");
    D3D11_SHADER_RESOURCE_VIEW_DESC srv_desc{};
    srv_desc.Format = texture_desc.Format;
    srv_desc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
    srv_desc.Texture2D.MipLevels = 1;
    throw_if_failed(g_d3d.device->CreateShaderResourceView(g_d3d.yuy2_texture, &srv_desc, &g_d3d.yuy2_srv), "Failed to create YUY2 shader resource view");
}
...
void initialize_d3d(HWND window) {
    ...
    const D3D_FEATURE_LEVEL requested_levels[] = {D3D_FEATURE_LEVEL_11_0, D3D_FEATURE_LEVEL_10_1, D3D_FEATURE_LEVEL_10_0};
    D3D_FEATURE_LEVEL created_level{};
    throw_if_failed(D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, 0, requested_levels,
                    static_cast<UINT>(sizeof(requested_levels) / sizeof(requested_levels[0])), D3D11_SDK_VERSION,
                    &swap_chain_desc, &g_d3d.swap_chain, &g_d3d.device, &created_level, &g_d3d.context),
    ...
    std::cout << "Direct3D initialized. Feature level: 0x" << std::hex << static_cast<unsigned>(created_level) << std::dec << '\n';
}
...
bool upload_latest_frame(CaptureSession& capture) {
    ...
    throw_if_failed(g_d3d.context->Map(g_d3d.yuy2_texture, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped), "Failed to map YUY2 upload texture");
    constexpr std::size_t source_row_bytes = static_cast<std::size_t>(CaptureSession::kCaptureWidth) * CaptureSession::kBytesPerPixel;
...
LRESULT CALLBACK window_proc(HWND window, UINT message, WPARAM w_param, LPARAM l_param) {
    ...
    case WM_SIZE:
        if (w_param != SIZE_MINIMIZED && g_d3d.swap_chain != nullptr) {
            try { resize_swap_chain(LOWORD(l_param), HIWORD(l_param)); }
...
HWND create_window(HINSTANCE instance) {
    ...
    if (RegisterClassExW(&window_class) == 0) throw std::runtime_error("Failed to register Win32 window class");
